...
export async function handleListImages(request: Request, runtimeEnv?: RuntimeEnv) {
    const url = new URL(request.url);
...
export async function handleUpdateImage(request: Request, id: string, runtimeEnv?: RuntimeEnv) {
...
export async function handleDeleteImage(id: string, runtimeEnv?: RuntimeEnv) {
    if (!isValidImageId(id)) {
        return json({ error: 'Invalid image id' }, 400);
    }
    return withGalleryTransaction(async () => {
