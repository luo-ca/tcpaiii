...
> 状态：**已实施**（第 9 节 3 个决策点均已拍板并落地，见第 10 节；本文件保留为规划与变更历史）
...
| Token | 值 | 用途 |
|---|---|---|
| `--radius-sm` | 8px | 小徽标、紧凑装饰条 |
| `--radius-md` | 10px | 标签、小按钮 |
| `--radius-lg` | 12px | 输入框、普通按钮 |
| `--radius-xl` | 16px | 卡内图标底座、次级容器 |
| `--radius-2xl` | 20px | 卡片、面板 |
| `--radius-3xl` | 24px | 大容器、预览面板 |
| `rounded-full` | 999px | 主 CTA、chip |
...
### 19.9 决策已定：保留六档圆角，规范 §4.2 同步为实际值

原争议：规范 §4.2 写的是三档（输入框/标签/小按钮 10px、卡片 16px、大容器 24px、CTA 胶囊），主题实际是六档（8/10/12/16/20/24）。

**决策：以代码为准，规范向代码看齐。** 理由：阶梯本身单调自洽，且降为三档要动 200+ 处 `rounded-*`，纯观感收益、回归风险不成比例。§4.2 表格已同步为六档实际值。
...
## 22. 首屏 CLS 归零：从「已达标」到「可避免的抖动全部消除」

第 14 节曾判定 CLS「已达标（桌面 0.048 / 手机 0.044~0.096）」并归因于
「瀑布流首访、后端不返回图片尺寸」。这一轮用同一套 `PerformanceObserver`
逐条读 `layout-shift` 的来源节点后，发现当时的归因**不完整**：
真正的位移全部来自**懒加载骨架的形状不匹配**与**数据到达后才挂载的区块**，
与图片尺寸无关，属于完全可避免的一类。
...
| 位置 | 症状（实测） | 根因 |
|---|---|---|
| 首页 Hero 统计行 | 左栏 421px → 535px，位移节点报 `.text-center.lg:text-left` | 写成 `{hasStatData && ...}`，stats 到达才挂载整行 |
| `/status` 懒加载 | 页脚 272 → 794px，**CLS 0.1657（越过 good 阈值 0.10）** | 通用 `SectionFallback`（约 256px）与真实页（778px）形状差得远 |
| `/docs` 懒加载 | 页脚下移 500px+，CLS 0.0801 | 同上，真实页 1206px |
| 首页实时统计 | 页脚 5112 → 5936px（下移 824px） | 同一 `SectionFallback`，真实区块 907~977px |

另外 `/status` 还有第二层：`StatusPage` **内部**的 loading 态也与真实内容不一致
（横幅 104 vs 79px、骨架行高 `h-4`(16px) vs `CheckRow` 的 `text-sm`(20px)、
存储绑定卡多画一行）。
...
| 改动 | 说明 |
|---|---|
| Hero 统计行 | 改成 `isPending \|\| hasStatData` 决定渲染，加载时渲染**同构骨架**：chip 数量/图标/文案/内边距全一致，数值位换成按各自量级定宽的骨架块（`2.7/1.6/2.7/4.1em`），因此 chip 宽度与换行在数据前后完全一致 |
| 新增 `StatusFallback` / `DocsFallback` / `RealtimeStatsFallback` | 按各页真实结构铺骨架，并删除已无引用的通用 `SectionFallback` |
| 骨架高度校准 | 同时考虑**断点**与 **CSS grid 的 stretch**：窄屏单列时三卡各自 166/132/196px，≥sm 变两列后同行被拉成等高（第二卡 132→166），骨架是固定高度、不参与拉伸，必须显式写 `sm:h-[166px]` |
| 内部 loading 态 | 横幅高度与真实一致（123/79px）；骨架行高改 `h-5`；「存储绑定」卡传 `rows={2}` |
...
| `/admin` | 0.0015 | 0.0015 |
| `/docs` | 0.0801 | **0** |
| `/status` | **0.1657** | **0 ~ 0.0024** |

多视口（320 / 390 / 768 / 1280）复核均落在此区间；唯一残留是 768px 下 `/status` 的 0.0024，远低于 good 阈值，属网格计算与固定骨架的亚像素差。

守卫见 `src/test/first-paint-cls.test.ts`（8 条，逐条对应上面每一处改动，
并已做「回退即转红」的有效性验证）。
