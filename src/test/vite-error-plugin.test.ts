...
function captureWindowListener(
  ...
  if (typeof handler === "function") {
    listeners[type].push(handler as CapturedListener);
    return;
  }

  listeners[type].push((event) => handler.handleEvent(event as Event));
}

function isRouteMessage(value: unknown): value is { type: "__VESA_ROUTE_CHANGE__"; path: string } {
  return (
    typeof value === "object" &&
    value !== null &&
    "type" in value &&
    value.type === "__VESA_ROUTE_CHANGE__"
  );
}
...
describe("vite-error-plugin: error script runtime behavior", () => {
  ...
  let listeners: Record<string, CapturedListener[]>;
  ...
  beforeEach(() => {
    ...
    vi.spyOn(window, "addEventListener").mockImplementation((type, handler) => {
      captureWindowListener(listeners, type, handler);
    });
...
describe("vite-error-plugin: route script runtime behavior", () => {
  ...
  it("should send initial route on script load", () => {
    const routeCalls = postMessageSpy.mock.calls.filter(
      ([message]) => isRouteMessage(message)
    );
