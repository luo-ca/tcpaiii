...
`npm install` 会自动装上 `pre-commit` 钩子（`prepare` 脚本，也会在
...
- `GET /api/health`：返回函数健康检查结果，便于确认 Pages Functions 是否正常启动。
...
函数源码维护在 `edge-functions-src/api/[[default]].ts`，`npm run build:functions` 会编译生成 EdgeOne 部署入口 `edge-functions/api/[[default]].js` 并接管 `/api/*`。代码优先读取 EdgeOne Pages 运行时注入的 `images_kv` / `stats_kv` 绑定，也兼容直接绑定为 `images` / `stats`；本地单元测试里仍可通过 mock 绑定覆盖 KV 行为。
...
如果线上访问 `/api/random?format=json` 返回 `<!doctype html>` 或首页内容，说明 `/api/*` 没有命中 Pages Functions，请检查 `edge-functions/api/[[default]].js` 是否随 EdgeOne Pages 项目部署，并确认当前域名指向的是新项目。
